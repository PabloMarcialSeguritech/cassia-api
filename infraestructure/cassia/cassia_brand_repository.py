...
async def get_brand_by_id(brand_id, db: DB):
    brand_df = None
    try:
        query_statement_get_brand_by_id = DBQueries().query_statement_get_brand_by_id
        brand_data = await db.run_query(query_statement_get_brand_by_id, (f"{brand_id}"))
        brand_df = pd.DataFrame(
            brand_data).replace(np.nan, None)
        return brand_df
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error en get_brand_by_id: {e}")
...
async def get_all_brands(db: DB):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al obtener los brands: {e}")
...
async def update_brand(brand_id, name_brand, brand_mac_address, db):
    try:
        query = DBQueries().builder_update_brand(
            brand_id, name_brand, brand_mac_address)
        await db.run_query(query)
        return True
    except Exception as e:
