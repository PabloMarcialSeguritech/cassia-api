...
async def get_global_alerts_by_tech_pool(tech_id, tipo, db):
    ...
    try:
        ...
        cassia_alerts_data = await db.run_query(sp_name_get_cassia_events_by_tech)
...
async def get_global_alerts_local_pool(db):
    try:
        sp_name_get_cassia_events_by_tech = DBQueries(
...
async def get_global_alerts_local_by_tech_id_pool(tech_host_id, db):
...
async def get_municipality_alerts_by_tech_pool(municipality, tech_id, tipo, db):
    ...
    try:
        ...
        cassia_alerts_data = await db.run_query(query_get_alerts_by_municipality_and_tech)
        cassia_alerts_df = pd.DataFrame(
            cassia_alerts_data).replace(np.nan, None)
        if not cassia_alerts_df.empty:
            cassia_alerts_df['r_eventid'] = ''
...
async def get_municipality_alerts_local_pool(municipality, db):
    try:
        query_get_alerts_by_municipality_and_tech = DBQueries(
...
async def get_cassia_events_acknowledges_pool(db):
    try:
        # PINK
        query_get_cassia_events_acknowledges = DBQueries(
        ).query_statement_get_cassia_events_acknowledges_test
        cassia_alerts_acknowledges_data = await db.run_query(query_get_cassia_events_acknowledges)
...
async def get_cassia_events_with_hosts_filter_pool(hostids: str, db):
    ...
    try:
        ...
        cassia_alerts_data = await db.run_query(query_get_cassia_events_with_hosts_filter)
        cassia_alerts_df = pd.DataFrame(
            cassia_alerts_data).replace(np.nan, None)
        return cassia_alerts_df
    except Exception as e:
